...
STYLES = (
    Style(
        ...
        (126, 150),
        ("four",),
        ("eights", "offbeat", "rolling16"),
        ("stab",),
        ("harm_minor", "minor", "phrygian"),
        (0.0, 0.08),
        ("hats", "riser", "roll"),
        "peak",
        0.25,
...
def make_sections(style: Style) -> list[Sect]:
    if style.structure == "peak":
        ...
        for cycle in range(2):
            ...
            out.append(
                Sect("build", int(pick((4, 8))), 0.85, True, True, False, False, True)
            )
            out.append(
                Sect(
                    "drop",
    ...
    for block in range(3):
        if block:
            out.append(
                Sect("lift", int(pick((2, 4))), 0.7, True, False, False, False, False)
            )
        out.append(
            Sect(
                "groove",
...
def make_song(first: bool) -> Song:
    ...
    return Song(
        ...
        sections=sections,
        kick_snd=kick(
            f0=float(RNG.uniform(250, 480)),
            punch=float(RNG.uniform(45, 80)),
...
def drums_into(buf: np.ndarray, song: Song, sect: Sect, beat: float, gbar: int) -> None:
    ...
    if song.groove == "four":
        ...
        for s in range(8):
            place(
                buf, hat(), t16(s * 2, beat, sw), (0.3 if s % 2 else 0.2) * g, pan=0.25
            )
        if gbar % 2 == 1:
    ...
    if gbar % 8 == 7 and g > 0.5:
        for s in range(8):
            place(
                buf,
                song.snare_snd,
                2 * beat + t16(s, beat, sw) / 2,
                (0.18 + 0.04 * s) * g,
            )


def bass_into(
    layer: np.ndarray, song: Song, chord: list[int], beat: float, gbar: int
) -> None:
    root = chord[0]
    st = song.bass_style
    if st == "eights":
        ...
        for s in range(16):
            place(
                layer,
                sub_bass(midi_freq(root), beat * 0.21),
                t16(s, beat, song.swing),
                0.3,
            )
    elif st == "funk16":
        if gbar // 8 != song.bass_key:
            ...
            slots = sorted(
                RNG.choice(16, size=int(RNG.integers(6, 10)), replace=False).tolist()
            )
            song.bass_pat = [(s, int(pick((0, 0, 0, 7, 12)))) for s in slots]
        for s, off in song.bass_pat:
            place(
                layer,
                sub_bass(midi_freq(root + off), beat * 0.22),
                t16(s, beat, song.swing),
                0.36,
            )
    elif st == "halftime":
...
def arp_into(
    song: Song, layer: np.ndarray, chord: list[int], beat: float, gbar: int, gain: float
) -> None:
    """Polymetric arp: an arp_len-step pattern read across 16-step bars."""
    if gbar // 8 != song.arp_key:
        song.arp_key = gbar // 8
        song.arp = [
            (int(RNG.integers(0, 4)), int(RNG.integers(0, 2)) * 12)
...
def melody_into(
    song: Song, layer: np.ndarray, beat: float, gbar: int, gain: float
) -> None:
    ...
    for s, deg, lenmult in song.motif:
        ...
        place(
            layer,
            voice(f, beat * 0.4 * lenmult),
            t16(s - half, beat, song.swing),
            gain,
            pan,
        )
        if canon and s - half < 12:
            place(
                layer,
                voice(f / 2, beat * 0.4 * lenmult),
                t16(s - half + 3, beat, song.swing),
                gain * 0.45,
                -pan,
            )
    ...
    if sect.kind == "trans":
        for s in range(16):
            place(
                ...
                song.snare_snd,
                s * beat / 4,
                0.2 + 0.35 * (bi * 16 + s) / (sect.bars * 16),
            )
        place(
            buf,
            riser(
                0.4 + 0.6 * bi / sect.bars, 0.4 + 0.6 * (bi + 1) / sect.bars, 4 * beat
            ),
            0,
            0.3,
        )
